...
async function main() {
    ...
    console.log('\n🔄 Propagando precios a consignación seller_inventory...')
    let sellerInventory = []
    page = 0
    hasMore = true
    while (hasMore) {
        const { data, error } = await supabase
            ...
            .select('id, source_product_id, base_reference_price_usd, status')
            .in('status', ['pending_reception', 'active', 'return_requested'])
            .range(page * PAGE_SIZE, (page + 1) * PAGE_SIZE - 1)

        if (error) {
            console.error('Error fetching seller_inventory:', error)
            break
        }

        if (data && data.length > 0) {
            sellerInventory = sellerInventory.concat(data)
            page++
            process.stdout.write(`\r   Seller rows cargadas: ${sellerInventory.length}...`)
            if (data.length < PAGE_SIZE) hasMore = false
        } else {
            hasMore = false
        }
    }
    console.log(`\n   ✅ Consignación cargada. Analizando ${sellerInventory.length} items.`)

    const productPriceById = new Map(productUpdates.map((p) => [p.id, Number(p.price_usd || 0)]))
    for (const p of localProducts) {
        if (!productPriceById.has(p.id)) productPriceById.set(p.id, Number(p.price_usd || 0))
    }

    const sellerUpdates = []
    for (const row of sellerInventory) {
        const sourceId = row.source_product_id
        if (!sourceId || !productPriceById.has(sourceId)) continue
        const nextPrice = Number(productPriceById.get(sourceId) || 0)
        if (!Number.isFinite(nextPrice) || nextPrice <= 0) continue
        const currentPrice = Number(row.base_reference_price_usd || 0)
        if (Math.abs(nextPrice - currentPrice) <= 0.01) continue
        sellerUpdates.push({ id: row.id, base_reference_price_usd: nextPrice })
    }

    let sellerUpdated = 0
    for (const row of sellerUpdates) {
        const { error } = await supabase
            .from('seller_inventory')
            .update({ base_reference_price_usd: row.base_reference_price_usd })
            .eq('id', row.id)
        if (!error) sellerUpdated++
        if (sellerUpdated % 50 === 0 && sellerUpdated > 0) {
            process.stdout.write(`\r   Seller actualizados: ${sellerUpdated} / ${sellerUpdates.length}`)
        }
    }
    if (sellerUpdates.length > 0) console.log('\n')
    console.log(`📊 CONSIGNACIÓN (seller_inventory):`)
    console.log(`   - Precios Actualizados: ${sellerUpdated}`)
    console.log(`   - Filas Analizadas: ${sellerInventory.length}`)
