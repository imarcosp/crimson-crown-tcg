...
The independently auditable [per-object evidence matrix](./crimson-migration-object-matrix-2026-08-29.json) records 160 object decisions underpinning all five remote pairs and all sixteen `baseline_present` results. Each row carries explicit expected and observed presence, an exact ordered signature, the ordered fields asserted by that decision, mismatched fields, and reciprocal source links. The matrix documents its canonical field and array ordering; it uses SHA-256 instead of plaintext for bodies, expressions, defaults, comments, and constraint definitions. Expressions, defaults, and constraint definitions are hashed after line-ending normalization and outer trim only; internal whitespace, case, qualification, casts, grouping, parentheses, and literal text are preserved. Its `strict-boundary-v1.1` section defines scanner states and every local/server payload boundary, anchors the nine local sources plus retained catalog, and records a 110-row replay with 0 hash changes, 0 result changes, and 14/14 scanner conformance cases. The source-set SHA-256 is `9680b9a4fe3055e9b3836542561c8bf21dcfae852e8b9b88bab4e0a5e337eb93`; the replay SHA-256 is `c656c2530df803ef33f2b2b19f586382576c3cde340b31973544251323c1321b`. The matrix contains 66 PASS and 94 FAIL object decisions. Its committed byte-level SHA-256 is `a0b4a71c89665aa25ec8160ad98f842339a9392810c6f537d369b17ede1de508`.
