...
| `mercadopago` | `2.12.0` | `2.12.0` | direct, exact exception pin |
| `mercadopago > uuid` | `9.0.1` | `9.0.1` | transitive exception |
...
| Check | Result |
| --- | --- |
| Dependency contract RED | 5 passed, 1 expected failure on unsafe/unpinned dependency versions |
| Dependency contract GREEN | 6/6 passed: exact resolution, Next response runtime, Axios GET/client, Puppeteer launch/PDF, Resend client/send, Mercado Pago v2 payment search |
| Release safety | 113/113 passed |
| Environment safety | 49/49 passed |
| Deployment safety | 7/7 passed |
| TypeScript | passed |
| Production build | passed with Next `16.3.3` and Webpack; 44 static/dynamic routes generated |
...
`mercadopago@2.12.0` declares `uuid@^9.0.0`; `uuid@11.1.1` is outside that supported range. The available audited remediation is `mercadopago@3.6.0`, a major SDK change. Mercado Pago remains disabled/deferred, its API behavior is unchanged, the SDK is pinned exactly to `2.12.0`, and the two remaining moderate audit entries are limited to the single `mercadopago@2.12.0 > uuid@9.0.1` chain identified by [GHSA-w5hq-g745-h8pq](https://github.com/advisories/GHSA-w5hq-g745-h8pq).
