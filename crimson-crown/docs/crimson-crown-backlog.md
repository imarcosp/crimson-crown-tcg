...
7. **Release observado.** La evidencia de reconciliación confirma Vercel `READY`, smoke público, 19 migraciones, 66 objetos de Storage y ausencia de cambios en órdenes, perfiles, productos e inventario fuera de las mutaciones explícitamente autorizadas.
...
- Filtros Magic completados: rango inclusivo sobre el precio final, formatos allowlisted con legalidades Scryfall, URL/paginación conservadas y validación responsive. La migración aditiva `20260830133000` está aplicada en producción; el backfill enriqueció 1.842 filas y el segundo plan confirmó cero cambios pendientes, cero faltantes y 1.842 filas sin cambios.
- Accesos rápidos de Home completados: modelo dinámico, orden, imagen o icono allowlisted, etiqueta, URL segura y estado activo; administración protegida por Server Actions/RLS y CRUD E2E con limpieza. La migración aditiva `20260830170000` está aplicada en producción; la tabla nació vacía y el Home conserva su fallback hasta que se cargue contenido explícitamente.
...
- La migración aditiva `20260830203000` crea únicamente las tres tablas del deckbuilder y una promoción atómica `service_role`; está aplicada en producción. Hay dos snapshots activos: Commander 8 decks/800 cartas y Modern 8 decks/242 cartas, para un total de 16 decks y 1.042 filas de cartas.
- Checkpoint local y productivo cerrado: matriz de seguridad, lint SQL, TypeScript, build de producción (46 páginas), suite release con 138 checks (137 pass y un skip de symlink propio de Windows) y E2E 25/25 en verde. En producción, productos 1.954, stock 2.287, órdenes 63, perfiles 81, inventarios 1 y movimientos 2 permanecieron idénticos tras las migraciones, snapshots y backfill.
