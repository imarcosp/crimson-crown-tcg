  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (mode === 'Magic' && !isRiftbound) {
        const params = new URLSearchParams({ q: query, inventory_id: inventoryId })
        fetch(`/api/search?${params.toString()}`).then((r) => r.json()).then((data) => setResults(data || [])).finally(() => setLoadingSearch(false))
      } else {
        const params = new URLSearchParams({ q: query, inventory_id: inventoryId })
        fetch(`/api/search?${params.toString()}`).then((r) => r.json()).then((data) => setResults((data || []).filter((d: any) => d.tcg === 'Riftbound'))).finally(() => setLoadingSearch(false))
      }
    }, 500)
    return () => clearTimeout(timer)
  }, [query, mode, isRiftbound, inventoryId])
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {mode === 'Magic' && (
            <div className="relative">
              <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Buscar carta en biblioteca Magic</label>
              <div className="relative">
              ...
              <p className="mt-1 text-[11px] text-slate-400">Busca coincidencias de la biblioteca de cartas y luego ajusta idioma, estado, acabado, stock o precio.</p>
              {results.length > 0 && (
                  ...
                  {results.map((r: any) => (
                    <button key={r.id} onClick={() => handleSelectCard(r)} className="w-full text-left p-2 hover:bg-slate-50 border-b last:border-0 flex items-center gap-3 transition-colors cursor-pointer">
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-[10px] font-bold uppercase tracking-wide text-purple-600">{r.search_source === 'external_prices' ? 'Biblioteca Magic' : r.search_source === 'scryfall' ? 'Scryfall' : 'En inventario'}</div>
                      </div>
                    </button>
                  ))}
                </div>
